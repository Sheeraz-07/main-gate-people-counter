...
if (!(Test-Path $venvPath)) {
    ...
    try {
        ...
        Write-Host "Make sure you have Python 3.10+ with venv module installed" -ForegroundColor Yellow
        exit 1
    }
} else {
...
try {
    ...
    Write-Host "Try deleting .venv folder and running script again" -ForegroundColor Yellow
    exit 1
}
...
if (Test-Path "requirements.txt") {
    ...
    try {
        pip install ultralytics opencv-python fastapi uvicorn aiosqlite pyyaml numpy --quiet
