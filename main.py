...
class PeopleCounterSystem:
    ...
    def _adjust_line_coordinates(self, config_coords, frame_width, frame_height):
        """Adjust counting line coordinates based on actual frame dimensions.
        
        The config coordinates are designed for substream resolution.
        This method scales them to match the actual camera resolution.
        """
        # Config assumes 640x480 resolution (typical for substream)
        config_width, config_height = 640, 480
        ...
        # Scale coordinates
        adjusted_x = int(config_coords[0] * width_scale)
        adjusted_y = int(config_coords[1] * height_scale)
        
        # Ensure coordinates are within frame bounds with some margin
        margin = 10
        adjusted_x = max(margin, min(adjusted_x, frame_width - margin))
        adjusted_y = max(margin, min(adjusted_y, frame_height - margin))
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Connect to get actual frame dimensions
            if not self.capture.connect():
                raise Exception("Failed to connect to video source")
            
            # Get a test frame to determine dimensions
            ret, test_frame = self.capture.read_frame()
            if ret and test_frame is not None:
                frame_height, frame_width = test_frame.shape[:2]
                self.logger.info(f"Detected frame dimensions: {frame_width}x{frame_height}")
                
                # Adjust counting line based on actual frame size
                counter_config = self.config['counter']
                line_start = self._adjust_line_coordinates(counter_config['line_start'], frame_width, frame_height)
                line_end = self._adjust_line_coordinates(counter_config['line_end'], frame_width, frame_height)
                
                self.logger.info(f"Adjusted counting line: {line_start} -> {line_end}")
            else:
                # Fallback to config values
                counter_config = self.config['counter']
                line_start = tuple(counter_config['line_start'])
                line_end = tuple(counter_config['line_end'])
                self.logger.warning("Could not get test frame, using config line coordinates")
            
            # Initialize counter with adjusted line
